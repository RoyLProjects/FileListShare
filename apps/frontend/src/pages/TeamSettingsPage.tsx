...
type Permission = paths["/v1/dashboard/teammember"]["patch"]["requestBody"]["content"]["application/json"]["permissions"][number];
type team = paths["/v1/dashboard/team"]["get"]["responses"]["200"]["content"]["application/json"]["data"]["items"][number];
...
const TeamSettingsPage: React.FC = () => {
  ...
  useEffect(() => {
    if (!loading) {
        if (teamError) {
        setErrorMessage(getErrorMessage(teamError) || "Failed to fetch team details");
    } else if (teamMembersError) {
        setErrorMessage(getErrorMessage(teamMembersError) || "Failed to fetch team members");
    } else if (storageError) {
        setErrorMessage(getErrorMessage(storageError) || "Failed to fetch storage details");
    }
...
>({
  ...
  onError: (err: unknown) => {
    ...
    setErrorMessage(getErrorMessage(err) || "Failed to save member permissions");
  },
});
...
const handleDeleteTeam = useMutation<
  unknown,
  unknown
>({
  mutationFn: async () => {
...
const loading = teamLoading || teamFetching || teamMembersLoading || teamMembersFetching || handleSaveMemberPermissions.isPending || handleRemoveMember.isPending || handleSaveTeamName.isPending || handleDeleteTeam.isPending || storageLoading || storageFetching || handleDeleteStorage.isPending;
  ...
  <div className="p-3 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-300 dark:border-red-700 text-sm font-medium text-red-700 dark:text-red-300">
    ...
    message={
      <>
        ...
        <span className="font-medium">{memberToRemove?.userId}</span> from the
